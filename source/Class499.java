/* Class499 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
import java.applet.Applet;

public class Class499
{
    public static final int anInt6838 = 1;
    static final int anInt6839 = 3;
    static final int anInt6840 = 0;
    static Applet anApplet6841 = null;
    static final int anInt6842 = 2;
    static String aString6843 = null;
    static Class40 aClass40_6844;
    public static int anInt6845;
    
    public static void method6072(Applet applet, String string) {
	anApplet6841 = applet;
	aString6843 = string;
    }
    
    Class499() throws Throwable {
	throw new Error();
    }
    
    public static final void method6073(byte i) {
	for (int i_0_ = 0; i_0_ < 5; i_0_++)
	    client.aBoolArray11139[i_0_] = false;
	client.anInt11065 = -681116073;
	client.anInt10988 = 159441195;
	Class215.anInt2272 = 0;
	Class480.anInt5455 = 0;
	Class390.anInt4111 = 2086833802;
	if (Class372.method4679(-1595827516) == 6) {
	    Class578 class578 = client.gameScene.method5966(1273279609);
	    int i_1_ = class578.anInt7607 * 961465569 << 9;
	    int i_2_ = -173815201 * class578.anInt7608 << 9;
	    int i_3_ = 1000 / Class262.method3646(-1701654150);
	    Class495.aClass283_Sub1_5578.method3760
		((float) i_3_ / 1000.0F,
		 (client.gameScene.method6024(-765386834)
		  .anIntArrayArrayArray5087),
		 client.gameScene.method6006((byte) 0), i_1_, i_2_,
		 -2137352661);
	}
	Class386.anInt4069 = 1476949137;
	Class690_Sub2.anInt10780 = 1603116895;
	client.anInt11136 = client.anInt11127 * 243491283;
	Class210.anInt2244 = 781637499 * Class626.anInt8136;
	Class375.anInt3938 = -823559375 * Class137.anInt1630;
	Class29.anInt306 = Class227.anInt2347 * -1751109343;
	Class424.anInt4789 = Class309_Sub1.anInt9907 * 715063449;
	Class278.anInt3052 = 323440981 * Class653.anInt8429;
	Class295.anInt3236 = client.anInt11156 * 1053713531;
    }
}
