/* Class603 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

public class Class603 implements Interface67
{
    int anInt7880;
    Class584 aClass584_7881 = Class584.aClass584_7698;
    Class364 aClass364_7882;
    Class446 aClass446_7883;
    Interface69 anInterface69_7884;
    
    public byte[] method430(int i) {
	int i_0_ = i;
	if (i_0_ == 0)
	    i_0_ = ((Class603) this).anInt7880 * -642889729;
	return ((Class603) this).aClass446_7883.method5395(i_0_, -349713035);
    }
    
    public void method309(int i) {
	if (((Class603) this).aClass584_7881 == Class584.aClass584_7699) {
	    method7183((byte) -10);
	    ((Class603) this).aClass584_7881 = Class584.aClass584_7700;
	    if (null != ((Class603) this).anInterface69_7884)
		((Class603) this).anInterface69_7884.method465
		    (this, 0, -642889729 * ((Class603) this).anInt7880, true,
		     -2043480297);
	}
    }
    
    public Class469 method438(int i) {
	Class469 class469 = new Class469(((Class603) this).aClass364_7882);
	class469.method5739(this, 937834506);
	return class469;
    }
    
    public int method454() {
	return 0;
    }
    
    void method7182() {
	/* empty */
    }
    
    public void method413(ByteBuffer class528_sub42) {
	/* empty */
    }
    
    public int method414(int i) {
	return 0;
    }
    
    public Class480 method453(int i) {
	return Class480.aClass480_5444;
    }
    
    public int method415(byte i) {
	return ((Class603) this).anInt7880 * -642889729;
    }
    
    public byte[] method416(int i, byte i_1_) {
	int i_2_ = i;
	if (i_2_ == 0)
	    i_2_ = ((Class603) this).anInt7880 * -642889729;
	return ((Class603) this).aClass446_7883.method5395(i_2_, -1565036452);
    }
    
    public void method423(ByteBuffer class528_sub42, int i) {
	/* empty */
    }
    
    public ByteBuffer method447(int i) {
	return null;
    }
    
    public boolean method419(int i, int i_3_) {
	return false;
    }
    
    public int method451() {
	return 0;
    }
    
    public int method420(int i) {
	return 0;
    }
    
    public void method421(boolean bool, int i) {
	/* empty */
    }
    
    public boolean method449(int i) {
	return false;
    }
    
    public void method337() {
	if (((Class603) this).aClass584_7881 == Class584.aClass584_7699) {
	    method7183((byte) -26);
	    ((Class603) this).aClass584_7881 = Class584.aClass584_7700;
	    if (null != ((Class603) this).anInterface69_7884)
		((Class603) this).anInterface69_7884.method465
		    (this, 0, -642889729 * ((Class603) this).anInt7880, true,
		     -711868015);
	}
    }
    
    public void method412(ByteBuffer class528_sub42) {
	/* empty */
    }
    
    public Class469 method445() {
	Class469 class469 = new Class469(((Class603) this).aClass364_7882);
	class469.method5739(this, 937834506);
	return class469;
    }
    
    public Class469 method424() {
	Class469 class469 = new Class469(((Class603) this).aClass364_7882);
	class469.method5739(this, 937834506);
	return class469;
    }
    
    public boolean method411(int i) {
	return true;
    }
    
    public boolean method426() {
	return true;
    }
    
    public Class603(Class446 class446, int i, Class364 class364,
		    Interface69 interface69) {
	((Class603) this).aClass446_7883 = class446;
	((Class603) this).anInt7880 = 840022015 * i;
	((Class603) this).aClass364_7882 = class364;
	((Class603) this).anInterface69_7884 = interface69;
	((Class603) this).aClass584_7881 = Class584.aClass584_7699;
    }
    
    public byte[] method428(int i) {
	int i_4_ = i;
	if (i_4_ == 0)
	    i_4_ = ((Class603) this).anInt7880 * -642889729;
	return ((Class603) this).aClass446_7883.method5395(i_4_, -1739705256);
    }
    
    public boolean method441() {
	return false;
    }
    
    public void method409(ByteBuffer class528_sub42) {
	/* empty */
    }
    
    public Class480 method431() {
	return Class480.aClass480_5444;
    }
    
    public Class480 method432() {
	return Class480.aClass480_5444;
    }
    
    public Class480 method433() {
	return Class480.aClass480_5444;
    }
    
    public Class480 method434() {
	return Class480.aClass480_5444;
    }
    
    public int method435() {
	return ((Class603) this).anInt7880 * -642889729;
    }
    
    public int method436() {
	return ((Class603) this).anInt7880 * -642889729;
    }
    
    public int method437() {
	return ((Class603) this).anInt7880 * -642889729;
    }
    
    public int method440() {
	return ((Class603) this).anInt7880 * -642889729;
    }
    
    public int method439() {
	return ((Class603) this).anInt7880 * -642889729;
    }
    
    void method7183(byte i) {
	/* empty */
    }
    
    public ByteBuffer method446(int i) {
	return null;
    }
    
    public void method452(ByteBuffer class528_sub42) {
	/* empty */
    }
    
    public Class584 method410(byte i) {
	return ((Class603) this).aClass584_7881;
    }
    
    public ByteBuffer method444(int i) {
	return null;
    }
    
    public ByteBuffer method422(int i) {
	return null;
    }
    
    public boolean method418(byte i) {
	return false;
    }
    
    public ByteBuffer method417(int i, byte i_5_) {
	return null;
    }
    
    public int method450() {
	return 0;
    }
    
    public boolean method443(int i) {
	return false;
    }
    
    public boolean method209() {
	return false;
    }
    
    public void method336() {
	if (((Class603) this).aClass584_7881 == Class584.aClass584_7699) {
	    method7183((byte) -56);
	    ((Class603) this).aClass584_7881 = Class584.aClass584_7700;
	    if (null != ((Class603) this).anInterface69_7884)
		((Class603) this).anInterface69_7884.method465
		    (this, 0, -642889729 * ((Class603) this).anInt7880, true,
		     623070567);
	}
    }
    
    public boolean method448(int i) {
	return false;
    }
    
    public int method442() {
	return 0;
    }
    
    public void method429(boolean bool) {
	/* empty */
    }
    
    public byte[] method427(int i) {
	int i_6_ = i;
	if (i_6_ == 0)
	    i_6_ = ((Class603) this).anInt7880 * -642889729;
	return ((Class603) this).aClass446_7883.method5395(i_6_, -1123382610);
    }
    
    public Class584 method455() {
	return ((Class603) this).aClass584_7881;
    }
    
    void method7184() {
	/* empty */
    }
    
    void method7185() {
	/* empty */
    }
    
    public boolean method425() {
	return true;
    }
    
    static Entity method7186(int i, int i_7_, int i_8_,
					      int i_9_) {
	Class546 class546 = (client.gameScene.method5973((byte) 62)
			     .aClass546ArrayArrayArray7160[i][i_7_][i_8_]);
	if (null == class546)
	    return null;
	Entity class645_sub1_sub5_sub1 = null;
	int i_10_ = -1;
	for (Class542 class542 = class546.aClass542_7399; class542 != null;
	     class542 = class542.aClass542_7238) {
	    Class645_Sub1_Sub5 class645_sub1_sub5
		= class542.aClass645_Sub1_Sub5_7239;
	    if (class645_sub1_sub5 instanceof Entity) {
		Entity class645_sub1_sub5_sub1_11_
		    = (Entity) class645_sub1_sub5;
		int i_12_
		    = ((class645_sub1_sub5_sub1_11_.method10671((byte) -92)
			- 1) * 256
		       + 252);
		Class422 class422
		    = class645_sub1_sub5_sub1_11_.method7693().aClass422_4868;
		int i_13_ = (int) class422.aFloat4780 - i_12_ >> 9;
		int i_14_ = (int) class422.aFloat4777 - i_12_ >> 9;
		int i_15_ = i_12_ + (int) class422.aFloat4780 >> 9;
		int i_16_ = (int) class422.aFloat4777 + i_12_ >> 9;
		if (i_13_ <= i_7_ && i_14_ <= i_8_ && i_15_ >= i_7_
		    && i_16_ >= i_8_) {
		    int i_17_ = (1 + i_15_ - i_7_) * (1 + i_16_ - i_8_);
		    if (i_17_ > i_10_) {
			class645_sub1_sub5_sub1 = class645_sub1_sub5_sub1_11_;
			i_10_ = i_17_;
		    }
		}
	    }
	}
	return class645_sub1_sub5_sub1;
    }
    
    static void method7187(int i, byte i_18_) {
	if (i == 6)
	    throw new Error();
	if (i == 14)
	    throw new OutOfMemoryError();
	try {
	    if (17 == i)
		Class301.method4022((byte) 8);
	    else if (16 == i)
		Class364_Sub2.method9236(new StringBuilder().append("").append
					     (296883021 * Class509.anInt6953)
					     .toString(),
					 (byte) 21);
	    else if (i == 22) {
		Class559 class559 = (client.gameScene.method5973
				     ((byte) 91).aClass559_7156);
		class559.aBool7491 = !class559.aBool7491;
	    } else if (i == 5)
		client.aBool10887 = true;
	    else if (i == 27)
		client.aBool10887 = false;
	    else if (13 == i)
		client.aClass695_11045.method8174(1372887195);
	    else if (9 == i) {
		Class625.method7450(-234974180);
		for (int i_19_ = 0; i_19_ < 10; i_19_++)
		    System.gc();
		Runtime runtime = Runtime.getRuntime();
		int i_20_
		    = (int) ((runtime.totalMemory() - runtime.freeMemory())
			     / 1024L);
		Class364_Sub2.method9236(new StringBuilder().append("").append
					     (i_20_).toString(),
					 (byte) 6);
	    } else if (19 == i) {
		Class625.method7450(-439562496);
		for (int i_21_ = 0; i_21_ < 10; i_21_++)
		    System.gc();
		Runtime runtime = Runtime.getRuntime();
		int i_22_
		    = (int) ((runtime.totalMemory() - runtime.freeMemory())
			     / 1024L);
		Class364_Sub2.method9236(new StringBuilder().append("").append
					     (i_22_).toString(),
					 (byte) -17);
		Class156.method1861(1285716496);
		Class625.method7450(-1335364486);
		for (int i_23_ = 0; i_23_ < 10; i_23_++)
		    System.gc();
		i_22_ = (int) ((runtime.totalMemory() - runtime.freeMemory())
			       / 1024L);
		Class364_Sub2.method9236(new StringBuilder().append("").append
					     (i_22_).toString(),
					 (byte) -16);
	    } else if (24 == i)
		Class364_Sub2.method9236((Class605.aClass525_7916
					      .method357((byte) 0)
					  ? "Success" : "Failure"),
					 (byte) -28);
	    else if (i == 25)
		Class12.aClass16_176.method634(860286429);
	    else if (i == 11)
		Class700.aClass454_8734.method5468(1962763505);
	    else if (20 == i)
		Class700.aClass454_8734.method5460((byte) 24);
	    else if (2 == i)
		Class64.aCanvas754.setLocation(50, 50);
	    else if (28 == i)
		Class64.aCanvas754.setLocation(1401768651 * Class509.anInt6931,
					       (Class509.anInt6943
						* -952966111));
	    else if (i == 15)
		Class181_Sub2.method8729((byte) 0);
	    else if (26 == i) {
		client.gameScene.aLong5558
		    = Class249.method3417(2017365772) * -8826400789005528605L;
		client.gameScene.aBool5574 = true;
		Class181_Sub2.method8729((byte) 0);
	    } else if (23 == i) {
		Class422 class422
		    = (Class108.myPlayer.method7693()
		       .aClass422_4868);
		Class364_Sub2.method9236(new StringBuilder().append
					     ((int) class422.aFloat4780 >> 9)
					     .append
					     (" ").append
					     ((int) class422.aFloat4777 >> 9)
					     .toString(),
					 (byte) -3);
	    } else if (8 == i) {
		Class422 class422
		    = (Class108.myPlayer.method7693()
		       .aClass422_4868);
		Class364_Sub2.method9236
		    (new StringBuilder().append("").append
			 (client.gameScene.method5973((byte) 18)
			      .aClass137Array7161
			      [(Class108.myPlayer
				.aByte10675)]
			      .method1669
			  ((int) class422.aFloat4780 >> 9,
			   (int) class422.aFloat4777 >> 9, (byte) 0))
			 .toString(),
		     (byte) -34);
	    } else if (29 == i) {
		Class364_Sub2.method9236(new StringBuilder().append
					     (Class229.aClass186_2490
						  .method2736(789585432))
					     .append
					     (" ").append
					     (Class229.aClass186_2490
						  .method2751(468297638))
					     .toString(),
					 (byte) 71);
		Class364_Sub2.method9236(new StringBuilder().append
					     (Class229.aClass186_2372
						  .method2736(421200957))
					     .append
					     (" ").append
					     (Class229.aClass186_2372
						  .method2751(811756251))
					     .toString(),
					 (byte) 40);
	    } else if (4 == i)
		Class666.method7908(false, -123298837);
	    else if (7 == i) {
		client.aBool11057 = !client.aBool11057;
		Class587.aClass173_7714.method2281(client.aBool11057);
		Class7.method566(-1313364045);
	    } else if (i == 10) {
		client.anInt10897 = 0;
		client.gameScene.method5989(2016078152);
	    } else if (i == 1) {
		client.anInt10897 = -1221863335;
		client.gameScene.method5989(2016078152);
	    } else if (21 == i) {
		client.anInt10897 = 1851240626;
		client.gameScene.method5989(2016078152);
	    }
	} catch (Exception exception) {
	    Class364_Sub2.method9236
		(Class39.aClass39_342.method807(Class378_Sub2.aClass668_10123,
						(byte) 102),
		 (byte) -8);
	}
    }
}
