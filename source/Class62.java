/* Class62 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

public class Class62
{
    public String aString713;
    public int anInt714;
    public String aString715;
    public int anInt716;
    public int anInt717;
    public boolean aBool718;
    public boolean aBool719;
    public String aString720;
    public String aString721;
    public int anInt722;
    
    Class62() {
	/* empty */
    }
    
    public static void method1021(int i) {
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub17_10585),
						 2, -112715264);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub17_10586),
						 2, 1918264188);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub4_10578),
						 1, 2092746993);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub23_10606),
						 1, 56172898);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub14_10570),
						 1, 704179099);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub11_10603),
						 1, 1897648535);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub7_10589),
						 1, 869007834);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub12_10590),
						 1, 93864618);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub8_10565),
						 2, 273204031);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub2_10580),
						 1, 70069309);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub15_10566),
						 2, 472700985);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub16_10576),
						 1, 1287711371);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub27_10567),
						 0, 763369377);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub27_10568),
						 0, 975912129);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub5_10579),
						 2, 860778716);
	Class514.aClass528_Sub38_6967.method9556
	    (Class514.aClass528_Sub38_6967.aClass691_Sub10_10572,
	     Class279.aClass279_3055.anInt3059 * -177612843, 234931805);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub19_10601),
						 0, 607650370);
	if (Class587.aClass173_7714 != null
	    && Class587.aClass173_7714.method2164()
	    && Class587.aClass173_7714.method2271())
	    Class587.aClass173_7714.method2231();
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub22_10588),
						 1, 2136774072);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub25_10592),
						 1, 1930511368);
	Class49.method886((byte) 89);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub13_10587),
						 0, -97609957);
	Class514.aClass528_Sub38_6967.method9556((Class514.aClass528_Sub38_6967
						  .aClass691_Sub35_10597),
						 4, 255697044);
	Class662.method7878(1119281887);
	client.aClass495_10935.method5975(40788639).method7492(1383331746);
	client.aBool10896 = true;
    }
    
    static int[] method1022(Class528_Sub21_Sub11 class528_sub21_sub11,
			    byte i) {
	int[] is = null;
	if (Class80.method1149(382057983 * ((Class528_Sub21_Sub11)
					    class528_sub21_sub11).anInt11595,
			       94305806))
	    is = ((ItemType)
		  (Class18.aClass5_Sub13_206.method63
		   ((int) (1856775008213551683L
			   * (((Class528_Sub21_Sub11) class528_sub21_sub11)
			      .aLong11589)),
		    -311497417))).anIntArray70;
	else if (-1
		 != -404794681 * (((Class528_Sub21_Sub11) class528_sub21_sub11)
				  .anInt11588))
	    is = ((ItemType)
		  (Class18.aClass5_Sub13_206.method63
		   (-404794681 * (((Class528_Sub21_Sub11) class528_sub21_sub11)
				  .anInt11588),
		    914097353))).anIntArray70;
	else if (Class12.method622((((Class528_Sub21_Sub11)
				     class528_sub21_sub11).anInt11595
				    * 382057983),
				   (byte) 0)) {
	    Class528_Sub31 class528_sub31
		= ((Class528_Sub31)
		   (client.aClass692_11110.method8137
		    ((long) (int) (1856775008213551683L
				   * ((Class528_Sub21_Sub11)
				      class528_sub21_sub11).aLong11589))));
	    if (null != class528_sub31) {
		NPC class645_sub1_sub5_sub1_sub1
		    = ((NPC)
		       class528_sub31.anObject10468);
		Class299 class299
		    = class645_sub1_sub5_sub1_sub1.aClass299_12043;
		if (class299.anIntArray3302 != null)
		    class299 = class299.method3996(Class570.playerVarsProvider,
						   Class570.playerVarsProvider,
						   -822661007);
		if (class299 != null)
		    is = class299.anIntArray3282;
	    }
	} else if (Class217.method3080(382057983 * (((Class528_Sub21_Sub11)
						     class528_sub21_sub11)
						    .anInt11595),
				       (byte) -66)) {
	    Class596 class596
		= ((Class596)
		   (client.aClass495_10935.method5976((short) 25792).method63
		    ((int) ((1856775008213551683L
			     * (((Class528_Sub21_Sub11) class528_sub21_sub11)
				.aLong11589)) >>> 32
			    & 0x7fffffffL),
		     1951741289)));
	    if (class596.anIntArray7767 != null)
		class596
		    = class596.method7099(Class570.playerVarsProvider,
					  Class570.playerVarsProvider, 339761117);
	    if (null != class596)
		is = class596.anIntArray7836;
	}
	return is;
    }
    
    public static void method1023(int i, int i_0_) {
	Class527.anInt7102 = 440417901 * i;
    }
    
    static final void method1024(Class648 class648, byte i) {
	int i_1_
	    = (((Class648) class648).anIntArray8394
	       [(((Class648) class648).anInt8395 -= 1239022665) * 717927929]);
	if (i_1_ < 0)
	    i_1_ = 0;
	client.anInt10981
	    = (client.anInt10980 * -2093894957 + i_1_) * 1996776135;
    }
}
